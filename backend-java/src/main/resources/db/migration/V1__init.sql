-- =========================================================================
-- VocabMaster 数据库初始化脚本  (Flyway V1)
-- =========================================================================
...
CREATE TABLE IF NOT EXISTS `user_settings` (
  ...
  `default_sort_mode` VARCHAR(10) NOT NULL DEFAULT 'alpha',
  `preferred_accent` VARCHAR(4) NOT NULL DEFAULT 'uk',
  `auto_play_audio` TINYINT NOT NULL DEFAULT 1,
  `notification_time` TIME DEFAULT '20:00:00',
  `theme` VARCHAR(10) NOT NULL DEFAULT 'light',
  `active_levels` JSON DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `word_bank` (
  ...
  `word_lower` VARCHAR(100) NOT NULL,
  ...
  `frequency` DECIMAL(4,3) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `user_word_progress` (
  ...
  `client_updated_at` DATETIME(3) DEFAULT NULL,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='学习行为日志';
...
INSERT IGNORE INTO `word_topic` (`code`, `name_zh`, `name_en`, `icon`, `sort_order`) VALUES
  ('FOOD', '食物饮食', 'Food', '🍔', 1),
  ('ANIMAL', '动物', 'Animal', '🐾', 2),
  ('PERSON', '人物', 'Person', '👤', 3),
...
INSERT IGNORE INTO `achievement` (`code`, `name_zh`, `description_zh`, `icon`, `category`, `trigger_rule`, `sort_order`) VALUES
